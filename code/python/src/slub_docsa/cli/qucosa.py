...
def available_qucosa_model_names(only_persistable: bool = False):
    """Return all persistable models for the qucosa dataset."""
    if only_persistable:
        return [n for n, m in default_qucosa_named_model_list() if isinstance(m(), PersistableClassificationModel)]
...
def load_qucosa_model_by_name(model_name):
    """Return a single model retrieving it by its name."""
    for name, model in default_qucosa_named_model_list():
        if name == model_name:
            return model()
