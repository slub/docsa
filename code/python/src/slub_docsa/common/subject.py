...
def print_subject_hierarchy(lang_code: str, subject_hierarchy: SubjectHierarchy):
    """Print a subject hierarchy to `stdout` using simple indentation."""
    for root_subject_uri in subject_hierarchy.root_subjects():
        ...
        while subject_backlog:
            ...
            current_label = subject_hierarchy.subject_labels(current_subject_uri).get(lang_code, "not available")
            print("    " * current_level, current_subject_uri, current_label)

            # find and add children for processing
            for children_subject_uri in subject_hierarchy.subject_children(current_subject_uri):
