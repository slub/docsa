...
def get_ann_classification_models_map() -> ModelTypeMapping:
    ...
    for lang_code in ["de", "en"]:
        models.update({
            f"tfidf_snowball_{lang_code}_10k_torch_ann": lambda subject_hierarchy, subject_order:
                TorchSingleLayerDenseReluModel(
                    ...
                    plot_training_history_filepath=os.path.join(
                        get_figures_dir(), f"ann_history/tfidf_snowball_{lang_code}_10k_torch_ann"
                    )
                ),
            f"tiny_bert_torch_ann_{lang_code}": lambda subject_hierarchy, subject_order:
                TorchBertModel(
                    vectorizer=get_static_wikipedia_wordpiece_vectorizer(
                        lang_code=lang_code, vocabulary_size=30000, max_length=64, uncased=True, limit=4000000
                    ),
                    batch_size=64,
                    max_epochs=None,
